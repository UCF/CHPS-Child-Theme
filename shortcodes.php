...
while($chpsnews->have_posts()) : $chpsnews->the_post();
	...
	if ( $postTerm && ! is_wp_error( $postTerm ) ) {
		$listhnews .= '<a class="category-title" href="hello' . $postTerm->term_id . '">' . $postTerm->name . '</a>';
	} else { 
...
$list = '<div class="' . $s['id'] . '" style="text-align: ' . $s['align'] . ';">';
...
if (!empty($s['tw'])) { $list .= '<a href="' . $s['tw'] . '" title="Follow Us On Twitter" target="_blank" class="tw-socialicon"><span class="fa-stack ' . $s['size'] . '"><i class="fa fa-circle fa-stack-2x"></i><i class="fa-brands fa-x-twitter fa-stack-1x fa-inverse"></i></span></a>'; } 
...
if( get_field('degrees') ) {
	...
	//END DEGREES
	if ( !empty($a['showjob'])) {
			if(get_field('job_titles_tax', get_the_ID())){	
...
if ( !empty($a['showunit'])) { }
	...
	// Get a list of terms for this post's custom taxonomy.
	$project_units = get_the_terms(get_the_ID(), 'lab_tax');
	// Renumber array.
...
$listlabs .= '<h4>';	
if( get_field('website_url') ) {		
	$listlabs .= '<a href="' . get_field('website_url') . '" target="_blank" rel="noopener noreferrer nofollow external" data-wpel-link="external">';
	}
...
$listlabs .= $content;
if( get_field('website_url') ) {	
	$listlabs .= '<div class="vc_btn3-container  btnhover-yellow vc_btn3-left mt-3" ><a style="background-color:#ffcc00; color:#000000;" class="vc_general vc_btn3 vc_btn3-size-md vc_btn3-shape-square vc_btn3-style-custom vc_btn3-icon-left" href="' . get_field('website_url') . '" title="" target="_blank" rel="nofollow external noopener noreferrer" data-wpel-link="external"><i class="vc_btn3-icon fa fa-external-link"></i> Visit the Lab&apos;s Website</a></div>';
}
